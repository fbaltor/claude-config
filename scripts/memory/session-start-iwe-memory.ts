...
const protocol = [
  ...
  "- **MCP tools (this session):** `iwe_find`, `iwe_retrieve`, `iwe_tree`, `iwe_stats`, `iwe_query` operate on the graph natively — prefer them. The `iwe …` CLI from `~/memory` is the exact equivalent.",
  "- Locate: `iwe find --fuzzy <query>` (title/key) or `iwe find --lexical <query>` (BM25 full-text on title+body).",
  "- Page in a branch: `iwe retrieve -k <key> --expand-includes 2 --expand-included-by 1` — the note + its inclusion children + parent context (backlinks on by default). Children are real edges, so `--expand-includes` walks the hub→leaf subtree; start at `iwe retrieve -k index --expand-includes 1` and descend. Budget with `-f keys` preview or `--max-tokens N`.",
