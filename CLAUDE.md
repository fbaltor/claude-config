...
- **The tiering has a floor, not just a ceiling.** Pinning the planner *up* to Fable is only half the split: without a floor, a Fable session runs Fable on the cheapest mechanical roles too, so the priciest model does the least valuable work. Mechanical, bounded roles are therefore pinned **down** in their own definitions (`dev-pipeline:coverage-verifier` → `model: sonnet`) rather than inheriting. Non-code phases (docs, config) are dispatched with an explicit cheap model.
- **Never downgrade a gate to work around an error.** Passing a `model` override *below* the session model to a reviewing role (`dev-pipeline:critic`, any agent dispatched *as* the cold reviewer) is prohibited — it silently trades away the rigour that is the gate's entire purpose. On a transient failure (429/529/overload), retry at the **same** tier, or resume the same agent via `SendMessage`; if the tier is genuinely unavailable, fall back **upward** per the rule below and say so. The critic self-reports its `model`/`effort` in `review_context` so a downgrade is visible in the report rather than invisible in a retry. Downgrading a *producing* role for cheap work (a docs writer) is fine; downgrading a *checking* role is not.
- **Effort is a per-role dial, not a global one.** `CLAUDE_CODE_EFFORT_LEVEL` is deliberately **absent** from `settings.json` `env`, so the session runs the platform default (`high` on every model that supports effort) and the user's `/effort` / `--effort` choice persists normally instead of being overridden every session. Each pipeline agent then sets its own `effort:` in frontmatter, which beats the session level. Default low and escalate on signal — `xhigh`/`max` on a bounded task costs latency and tokens without changing the answer. Pinning a global `xhigh` is the anti-pattern: measured on 2026-07-29 it applied maximum-but-one reasoning to read-only audits and README edits and made thinking 65–82% of every token generated (`~/.claude/research/2026-07-29-dev-pipeline-throughput-audit.md`). Claude does not set this env var back; effort is the user's lever, like `/model`.
